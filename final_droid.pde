...
// MonsterDraw v1.0 android build


// these allow for taking screen shots on an android device:
import android.content.Intent;
import android.net.Uri;
import android.os.Environment;

// this allows the app to make a file directory if it does not exist.
import java.io.File; 

// this lets an android device know that you might want to have it update the file that
// it uses to index and show images. Otherwise you can take a picture, and the file will
// be where ever it is save, but you won't see it on your fancy android picture viewing apps
...
void setup() 
{
  ...
  // get maxim up and running.
  maxim = new Maxim(this);
  
  // load the startup sound
  startUp = maxim.loadFile("OpenSound.wav");
  startUp.setLooping(false);
  ...
  backgroundButton = new RectButton((int(width - (width * 0.0859375))), (int(height / 1.463)), (int(width / 12.8)), (int(width / 12.8)), "BackgroundStrip.png", 2, "BackgroundButtonSound.wav",1);
  clearButton = new RectButton((int(width - (width * 0.08517))), (int(height / 1.217)), (int(height / 12)), (int(height / 12)), "ClearStrip.png", 2, "ClearButtonSound.wav",1);
  colorButton = new RectButton((int(width - (width * 0.0859375))), (int(height * 0.518)), (int(width / 12.8)), (int(width / 12.8)), "ColorStrip.png", 6, "ColorButtonSound.wav",1);
  brushButton = new RectButton((int(width - (width * 0.0859375))), (int(height / 2.83)), (int(width / 12.8)), (int(width / 12.8)), "BrushStrip.png", 9, "BrushButtonSound.wav",3);
  ...
  exitButton = new CircleButton((int(width * 0.05078)), (int(height * 0.9125)),(int(0.0219 * width)), "ExitStrip.png", 2, "ExitButtonSound.wav", 1);
  soundButton = new CircleButton((int(width / 16)), (int(height / 9)), (int(height / 11.8)), "SoundStrip.png", 2, "SoundButtonSound.wav", 1);
  screenShotButton = new CircleButton((int(width - (width / 15.8))), (int(height / 9)), (int(height / 11.8)), "CameraStrip.png", 2, "CameraButtonSound.wav", 1);
  screenShotTaken = new CircleButton((int(width - (width / 15.8))), (int(height / 9)), (int(height / 11.8)), "PhotoTaken.png", 1, "CameraButtonSound.wav", 1);
  ...
  if (!(mouseX > (border) && mouseX < (width - (border ))
      ...
      && pmouseY > (bottomBorder) && pmouseY < (height - (bottomBorder))))
      {
        ...
        if (screenShotReturn != screenShotHandler)
        {
          ...
          // for Android:
          // it is currently beyond my time constraints to figure out an alternate for the get() method that
          // works on an Android device. So android screen shots will need to be saved with the GUI boarder attached.
          
          // makes a new directory in Picture for the screen shots if it doesn't exist.
          File f = new File("//sdcard/Pictures/Processing/");
          f.mkdir();
          
          // saves the new image to the directory /Pictures/Processing
          save("//sdcard/Pictures/Processing/" + year() + "-" + month() + "-" + day() + " " + hour() + "." + minute() + "." +  second() + ".jpg");
          
          // makes use of that intent code above to have the android device reindex the photos!
          sendBroadcast(intent);          
          
        }
        
      // This ends the area constraint in mousePressed()
      }
